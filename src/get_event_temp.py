...
def main():
    ...
    working_data = pd.DataFrame(data=None, index=None, columns=['タイトル', '開始日時', '終了日時', '時間'], dtype=None, copy=False)
    ...
    for event in events:
        ...
        # print(start, end, working_time, event['summary'])

        tmp = pd.Series([event['summary'], start, end, working_time], index=working_data.columns)
